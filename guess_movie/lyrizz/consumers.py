...
from .views import Game, Question, Song, Answer, Lyrics
...
class GameMasterConsumerLyrizz(AsyncWebsocketConsumer):
    ...
    def get_data(self, current_q):
        ...
        i_lyrics = q.i_lyrics
        all_lyrics_text = Lyrics.objects.filter(song=q.song_guessed).all()
        lyrics = all_lyrics_text[i_lyrics:i_lyrics + self.game.size]
        lyrics_text = [l.lyrics_text for l in lyrics]
        ...
        context = {
            ...
            'song2_id': q.song2.id,
            'song3_id': q.song3.id,
            'song1_name': q.song1.artists + ' - ' + q.song1.name,
            'song2_name': q.song2.artists + ' - ' + q.song2.name,
            'song3_name': q.song3.artists + ' - ' + q.song3.name,
            'song1_url': str(q.song1.image),
            'song2_url': str(q.song2.image),
            'song3_url': str(q.song3.image),
            'lyrics_text': json.dumps(lyrics_text),
    ...
    def compute_score(self, i):
        ...
        for user_id in self.list_id:
            count = Answer.objects.filter(question_id=q.id, song_prop_id=q.song_guessed.id, user_id=user_id).count()
    ...
    @database_sync_to_async
    def is_correct(self, i, song_id):
        q = self.questions[i]
        if q.song_guessed.id == int(song_id):
            return 1
        else:
            return 0
    ...
    async def newanswer(self, event):
        if event['code'] == 'new_answer':
            ...
            ans = await self.is_correct(self.current_q, event['song_id'])
...
class UserConsumerLyrizz(AsyncWebsocketConsumer):
    async def connect(self):
...
class GameMasterConsumerHomeLyrizz(AsyncWebsocketConsumer):
    async def connect(self):
        self.user_id = self.scope['url_route']['kwargs']['user_id']
        self.room_name = self.scope['url_route']['kwargs']['room_name']
...
class UserConsumerHomeLyrizz(AsyncWebsocketConsumer):
    async def connect(self):
        # self.user_id = self.scope['url_route']['kwargs']['user_id']
...
class GameMasterConsumerResultsLyrizz(AsyncWebsocketConsumer):
    async def connect(self):
        # self.user_id = self.scope['url_route']['kwargs']['user_id']
        self.room_name = self.scope['url_route']['kwargs']['room_name']
...
class UserConsumerResultsLyrizz(AsyncWebsocketConsumer):
    async def connect(self):
        self.room_name = self.scope['url_route']['kwargs']['room_name']
        self.room_group_name = 'results_%s' % self.room_name
